...
class PrivacyEngine:
    ...
    def step(self, is_empty: bool = False):
        ...
        for p, clip_value in zip(params, clip_values):
            # print("1 p.grad", torch.max(p.grad))
            if self.rank == 0:
                ...
                # print("2 prenoise", torch.max(noise))

                if self.loss_reduction == "mean":
                    noise /= batch_size
                # print("2 noise", torch.max(noise))
                # p.grad += torch.clip(noise, p.grad.min(), p.grad.max())
                p.grad += noise
                # print("3 p.grad", torch.max(p.grad))
