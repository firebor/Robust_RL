...
def draw_msr_norm(env_name, algorithm_params):
    """
    绘制归一化后的均值和标准差折线图，对比不同算法。
    每个算法的结果是其在指定种子上的聚合，并基于各自的基线进行归一化。

    参数:
    env_name: 环境名称
    algorithm_params: 包含所有算法参数的字典。
    """
    print("开始绘制 MSR Norm (Normalized Mean ± Std) 图...")
    # 确定保存路径
    prefix_path = f'./res/msr_norm/{env_name}'
    ...
    baselines = {}
    valid_algorithms = []
    for algo_name in list(all_rewards.keys()): # 使用 list 迭代，因为可能在循环中删除元素
        if all_rewards[algo_name]['mass']: # 检查是否有数据
            mass_rewards_np = np.array(all_rewards[algo_name]['mass'])
            friction_rewards_np = np.array(all_rewards[algo_name]['friction'])
            all_rewards[algo_name]['mass'] = mass_rewards_np
            all_rewards[algo_name]['friction'] = friction_rewards_np
            
            # 计算基准值 (假设基准是索引为5的值的均值，即值为1.0的点)
            # 需要确保索引5存在且数据有效
            if mass_rewards_np.shape[1] > 5 and friction_rewards_np.shape[1] > 5:
                baseline_mass = np.mean(mass_rewards_np[:, 5])
                baseline_friction = np.mean(friction_rewards_np[:, 5])
                
                # 避免除以零
                baselines[algo_name] = {
                    'mass': baseline_mass if baseline_mass != 0 else 1.0,
                    'friction': baseline_friction if baseline_friction != 0 else 1.0
                }
                valid_algorithms.append(algo_name)
            else:
                print(f"  警告: 算法 {algo_name} 的聚合数据形状不符合预期，无法计算基线，将不参与绘图。")
                del all_rewards[algo_name]
        else:
            # 如果某个算法没有加载到任何数据，从字典中移除
            del all_rewards[algo_name]
            print(f"  算法 {algo_name} 没有加载到有效数据，将不参与绘图。")

    if not valid_algorithms:
        print("错误: 没有为任何算法加载到有效数据或计算出有效基线，无法绘制 MSR Norm 图。")
        return
    ...
    for algo_name in valid_algorithms:
        ...
        baseline = baselines[algo_name]['mass']
        if rewards['mass'].size > 0: 
            # 计算归一化后的均值和标准差
            mean = np.mean(rewards['mass'], axis=0) / baseline
            std = np.std(rewards['mass'], axis=0) / baseline
    ...
    if plot_count > 0:
        plt.title(f'{env_name} - Normalized Reward vs Mass (Mean ± Std across seeds)', fontsize=16)
        plt.xlabel('Mass', fontsize=14)
        plt.ylabel('Normalized Reward', fontsize=14)
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.legend(fontsize=10)
        save_path_mass = f'{prefix_path}/Mass/normalized_reward_vs_mass_mean_std.pdf'
    ...
    for algo_name in valid_algorithms:
        rewards = all_rewards[algo_name]
        baseline = baselines[algo_name]['friction']
        if rewards['friction'].size > 0:
            # 计算归一化后的均值和标准差
            mean = np.mean(rewards['friction'], axis=0) / baseline
            std = np.std(rewards['friction'], axis=0) / baseline
    ...
    if plot_count > 0:
        plt.title(f'{env_name} - Normalized Reward vs Friction (Mean ± Std across seeds)', fontsize=16)
        plt.xlabel('Friction', fontsize=14)
        plt.ylabel('Normalized Reward', fontsize=14)
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.legend(fontsize=10)
        save_path_friction = f'{prefix_path}/Friction/normalized_reward_vs_friction_mean_std.pdf'
        plt.savefig(save_path_friction, bbox_inches='tight')
        print(f"  图像已保存到 {save_path_friction}")
    plt.close()
    print("MSR Norm 图绘制完成。")

# 移除 draw_msr_norm_with_seed 函数
# def draw_msr_norm_with_seed(...):
#     pass