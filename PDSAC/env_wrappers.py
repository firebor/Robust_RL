...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        elif cmd == "get_feature_name_info":
            remote.send(env.obs_features_name)
        else:
            raise NotImplementedError
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        self.remotes[0].send(("get_feature_name_info", None))
        self.obs_features_name = self.remotes[0].recv()
...
# 新增：用于多进程测试的环境包装器
class MultiProcVecEnv(ShareVecEnv):
    def __init__(self, env_fns, num_processes=50):
        """
        初始化多进程环境包装器
        env_fns: 环境创建函数列表
        num_processes: 并行进程数
        ...
        # 处理标准Gym环境
        if not hasattr(env, 'share_observation_space'):
            env.share_observation_space = env.observation_space
            
        ShareVecEnv.__init__(self, num_processes, env.observation_space, env.share_observation_space, env.action_space)
    ...
    def step_wait(self):
        ...
        for i, (env, action) in enumerate(zip(self.envs, self.actions)):
            ...
            if isinstance(action, np.ndarray):
                action = action.reshape(-1)  # 将动作展平为一维数组
            # 处理新版本Gym环境的返回值
            step_result = env.step(action)
            if len(step_result) == 5:  # 新版本Gym环境
            ...
            # 对于标准Gym环境，使用相同的观察空间
            s_obs = obs
            available_actions = np.ones(env.action_space.shape[0])  # 假设所有动作都可用
            results.append((obs, s_obs, reward, done, info, available_actions))
            
            if done:
                reset_result = env.reset()
                if isinstance(reset_result, tuple):  # 新版本Gym环境
                    obs, _ = reset_result
                else:  # 旧版本Gym环境
                    obs = reset_result
                s_obs = obs
                results[-1] = (obs, s_obs, reward, done, info, available_actions)
    ...
    def reset(self):
        ...
        for env in self.envs:
            ...
            if isinstance(reset_result, tuple):  # 新版本Gym环境
                ...
                obs = reset_result
            s_obs = obs  # 对于标准Gym环境，使用相同的观察空间
            available_actions = np.ones(env.action_space.shape[0])  # 假设所有动作都可用
            results.append((obs, s_obs, available_actions))
