...
def main(args, agent_args):
    ...
    os.makedirs(f'./train_result/{args.env_name}', exist_ok=True)
    # 日志设置
    algo_name = args.algo
    if args.use_robust and args.algo == 'sac':
    ...
    # 根据参数选择使用普通SAC还是鲁棒SAC
    if args.use_robust and args.algo == 'sac':
        # 为RobustSAC添加额外参数
        agent_args.sigma = args.sigma
        agent_args.hessian_weight = args.hessian_weight
        agent = RobustSAC(writer, device, state_dim, action_dim, agent_args)
        logger.info(f"使用鲁棒SAC算法，seed={args.seed}, noise={args.sigma}")
    else:
        agent = SAC(writer, device, state_dim, action_dim, agent_args)
    if args.load != 'no':
...
def main_test(args, agent_args):
    ...
    if args.use_robust and args.algo == 'sac':
        ...
        agent_args.hessian_weight = args.hessian_weight
        agent = RobustSAC(writer, device, state_dim, action_dim, agent_args)
        print(f"使用鲁棒SAC算法，sigma={args.sigma}, hessian_weight={args.hessian_weight}")
        logger.info(f"使用鲁棒SAC算法，sigma={args.sigma}, hessian_weight={args.hessian_weight}")
    else:
        agent = SAC(writer, device, state_dim, action_dim, agent_args)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--seed", type=int, default=1008611111, help="随机种子(默认42)")
    parser.add_argument("--use_robust", type=bool, default=True, help='是否使用鲁棒SAC(default : False)')
    parser.add_argument("--sigma", type=float, default=0.1, help='鲁棒SAC的sigma(default : 0.1)')
    parser.add_argument("--hessian_weight", type=float, default=1, help='鲁棒SAC的hessian_weight(default : 0.1)')
    ...
    parser = ConfigParser()
    parser.read('/home/admin1/code/Robust-RL-1/HSSAC/config.ini')
