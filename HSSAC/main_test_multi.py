...
def worker(args):
    ...
    """
    try:
        env_name, model_path, mass, friction, state_dim, action_dim = args
        ...
        agent = RobustSAC(None, device, state_dim, action_dim, agent_args)
...
def evaluate_policy(env, model_path, state_dim, action_dim, mass=None, friction=None):
    """
    并行评估策略
    每次运行20个进程，运行5次来完成100次评估
    """
    ...
    for batch in range(5):
        ...
        tasks = [(env_name, model_path, mass, friction, 
                 state_dim, action_dim) for _ in range(20)]
        ...
        try:
            ...
            print(f"Batch {batch} error: {str(e)}")
            raise e
        finally:
            # 确保资源被正确释放
            pool.close()
    ...
    return int(total_reward / 100)  # 总奖励除以100得到平均值


def test_policy(env_name, model_path, state_dim, action_dim, agent_args, 
    ...
    for mass in mass_list:
        ...
        reward = evaluate_policy(env, model_path, state_dim, action_dim, mass=mass)
    ...
    for friction in friction_list:
        ...
        reward = evaluate_policy(env, model_path, state_dim, action_dim, friction=friction)
...
def main_test(args, agent_args):
    ...
    test_policy(args.env_name, model_path, state_dim, action_dim, agent_args, 
