...
def test_policy(env_name, agent, DISTURBANCE_DICT, writer, seed=42, logger=None, exp_name='Dynamic_weights'):
    # 设置固定的测试种子
    TEST_SEED = seed
    ...
    np.save(f'./test_result/{exp_name}/{env_name}/result_numpy/{env_name}_seed{seed}.npy',
...
def main_test(args, agent_args):
    ...
    # 创建日志目录
    algo_name = 'robust_sac'
    log_dir_temp = f'{prefix_dir}/test_tensorboard_logs/{args.env_name}_{algo_name}_seed{args.seed}'
    log_dir = f'{prefix_dir}/test_tensorboard_logs/{args.env_name}_{algo_name}_seed{args.seed}'
    counter = 0
    ...
    log_log_dir = ""
    if counter == 0:
        log_log_dir = f'{prefix_dir}/test_logs_logs/{args.env_name}_{algo_name}_seed{args.seed}.log'
    else:
        log_log_dir = f'{prefix_dir}/test_logs_logs/{args.env_name}_{algo_name}_seed{args.seed}({counter}).log'
    ...
    # 为RobustSAC添加额外参数
    agent_args.sigma = 1
    agent_args.hessian_weight = 1
    agent = RobustSAC(writer, device, state_dim, action_dim, agent_args)
    print(f"使用鲁棒SAC算法，seed={args.seed}")
    logger.info(f"使用鲁棒SAC算法，seed={args.seed}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--exp_name", type=str, default='Dynamic_weights', help='实验名称，用于保存结果')
    ...
    # parser.add_argument("--use_robust", type=bool, default=False, help='use robust SAC(default : False)')
    # parser.add_argument("--sigma", type=float, default=0.1, help='sigma for robust SAC(default : 0.5)')
    # parser.add_argument("--hessian_weight", type=float, default=1, help='hessian_weight for robust SAC(default : 0.5)')
