...
def worker(args):
    ...
    """
    try:
        env_name, model_state_dict, mass, friction, state_dim, action_dim, noise_multiplier = args
...
def evaluate_policy(env_name, model_state_dict, state_dim, action_dim, noise_multiplier, mass=None, friction=None, num_processes=16):
    """
    为特定的质量和摩擦力配置创建并运行一个worker进程
    """
    ...
    with Pool(processes=1) as pool:
        # 准备任务参数
        args = (env_name, model_state_dict, mass, friction, state_dim, action_dim, noise_multiplier)
        
        # 执行评估
        result = pool.apply(worker, (args,))
        
        # 返回评估结果
        _, _, reward = result
        return int(reward)  # 转换为整数

def test_policy(env_name, model_state_dict, state_dim, action_dim, noise_multiplier, agent_args, 
                DISTURBANCE_DICT, writer, seed=42, logger=None, exp_name='mutil_test', switch_steps=60):
    """
    使用多进程测试不同的质量和摩擦力组合
    """
    ...
    for mass in mass_list:
        for friction in friction_list:
            tasks.append((env_name, model_state_dict, mass, friction, state_dim, action_dim, noise_multiplier))
    
    # 创建进程池并行处理所有任务
    from multiprocessing import Pool
    
    print(f"开始并行测试 ({len(mass_list)}x{len(friction_list)} 网格点)，使用最多35个进程...")
    
    # 使用进程池
    total_tasks = len(tasks)
    with Pool(processes=30) as pool:  # 使用25个进程
        # 并行执行评估
        results = []
...
def main_test(args, agent_args):
    ...
    model_path = get_model_path(args.env_name,args.seed,args.exp_name, args.switch_steps)
    ...
    DISTURBANCE_DICT = {
        ...
        # "Walker2d-v2": {"mass_list": np.linspace(0.1,0.2, 2), "friction_list": np.linspace(0.1,0.2,2)},
    ...
    log_dir_temp=f'{prefix_dir}/test_tensorboard_logs/{args.env_name}_{args.algo}_seed{args.seed}_sw{args.switch_steps}'
    log_dir=f'{prefix_dir}/test_tensorboard_logs/{args.env_name}_{args.algo}_seed{args.seed}_sw{args.switch_steps}'
    counter=0
    ...
    log_log_dir=""
    if counter==0:
        log_log_dir=f'{prefix_dir}/test_logs_logs/{args.env_name}_{args.algo}_seed{args.seed}_sw{args.switch_steps}.log'
    else:
        log_log_dir=f'{prefix_dir}/test_logs_logs/{args.env_name}_{args.algo}_seed{args.seed}_sw{args.switch_steps}({counter}).log'
    ...
    test_policy(args.env_name, model_state_dict, state_dim, action_dim, args.noise_multiplier, agent_args, 
                DISTURBANCE_DICT, writer, seed=args.seed, logger=logger, exp_name=args.exp_name, switch_steps=args.switch_steps)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--exp_name", type=str, default='RLE', help='实验名称，用于保存结果')
    parser.add_argument("--seed", type=int, default=42, help="随机种子(默认42)")
    parser.add_argument("--switch_steps", type=int, default=60, help="加噪声的步数")
