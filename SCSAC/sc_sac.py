...
class SCSAC(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, args):
        super(SCSAC, self).__init__()
        self.args = args
    ...
    def get_sc_q(self, q, soft_q, states, target):
        gradients = grad(outputs=q, inputs=states,
                         grad_outputs=self.noise_weights_func(q.size()), only_inputs=True, create_graph=target)[0]
        gradient_norm = norm(gradients, p=1, dim=-1, keepdim=True)  # 换成 1 范数
        state_noise = self._state_noise
        sc_q = soft_q - state_noise * gradient_norm
        # sc_q = soft_q
        return sc_q
    ...
    def q_update(self, Q, q_optimizer, states, actions, rewards, next_states, dones):
        ...
        next_states = next_states.requires_grad_()
        q_1 = self.target_q_1(next_states, next_actions)
        q_2 = self.target_q_2(next_states, next_actions)
        q = torch.min(q_1, q_2)
        soft_q = q - self.alpha * next_action_log_prob
        sc_q = self.get_sc_q(q, soft_q, next_states, target=False)
        v = (1 - dones) * sc_q.detach()
    ...
    def actor_update(self, states):
        ...
        q = torch.min(q_1, q_2)
        soft_q = q - self.alpha.detach() * now_action_log_prob
        sc_q = self.get_sc_q(q, soft_q, states, target=True)
