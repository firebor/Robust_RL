...
def test_policy(env_name, model_state_dict, norm_stats, state_dim, action_dim, 
                DISTURBANCE_DICT, seed=42, logger=None, exp_name='nippo_test'):
    """
    使用多进程测试不同的质量和摩擦力组合
    """
    ...
    for folder_name in os.listdir(model_dir):
        ...
        if os.path.isdir(folder_path) and match and match.group(1) == str(seed):
            ...
            if files:
                ...
                norm_stats_dir = f'./train_result/{exp_name}/{env_name}/normalization_stats/{env_name}_PPO_seed{seed}'
...
def main_test(test_args):
    ...
    model_path, norm_stats = get_model_path(test_args.env_name, test_args.seed, test_args.exp_name)
    ...
    # 创建日志目录
    log_dir_temp = f'{prefix_dir}/test_tensorboard_logs/{test_args.env_name}_PPO_seed{test_args.seed}'
    ...
    if counter == 0:
        log_log_dir = f'{prefix_dir}/test_logs_logs/{test_args.env_name}_PPO_seed{test_args.seed}.log'
    else:
        log_log_dir = f'{prefix_dir}/test_logs_logs/{test_args.env_name}_PPO_seed{test_args.seed}({counter}).log'
