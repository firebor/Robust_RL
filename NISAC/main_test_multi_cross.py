...
import numpy as np
import os
from noise_injected_sac import NISAC
...
def worker(args):
    ...
    try:
        ...
        # 创建智能体
        agent = NISAC(None, device, state_dim, action_dim, noise_multiplier, agent_args)
...
def test_policy(env_name, model_state_dict, state_dim, action_dim, noise_multiplier, agent_args, 
    ...
    np.save(f'./test_result/{exp_name}_cross/{env_name}/result_numpy/{env_name}_seed{seed}_noise{noise_multiplier}.npy',
...
def main_test(args, agent_args):
    ...
    DISTURBANCE_DICT = {
        ...
        "InvertedDoublePendulum-v2": {"mass_list": np.linspace(0.1,1.9,51),
                                      "friction_list": np.linspace(0.1,1.9,51)}}
...
if __name__ == '__main__':
    ...
    parser.add_argument("--exp_name", type=str, default='norm_sacled_noise_cross', help='实验名称，用于保存结果')
    parser.add_argument("--noise_multiplier", type=float, default=1.0, help="设置高斯噪声的标准差(默认0.2)")
    parser.add_argument("--seed", type=int, default=0, help="随机种子(默认42)")
