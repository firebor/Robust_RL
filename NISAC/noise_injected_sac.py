...
class NISAC(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, noise_multiplier, args):
        ...
        self.noise_amplitude = noise_multiplier
        ...
        # # 将 PrivacyEngine 绑定到优化器上
        # self.privacy_engine_actor.attach(self.actor_optimizer)
    ...
    def actor_update(self, states):
        ...
        # Add Gaussian noise to gradients manually
        ...
        grad_norm = torch.nn.utils.clip_grad_norm_(self.actor.parameters(), max_norm=1)
        self.writer.add_scalar("grad_norm/actor", grad_norm, self.n_epi)
        self.n_epi += 1
    ...
    def train_net(self, batch_size, steps):
        ...
        ### actor update
        #  self.noise_amplitude = 1/(1 + torch.exp(torch.tensor(1e-4 * (steps - 1e6)))) / batch_size
        self.noise_amplitude = torch.sqrt(torch.tensor(self.noise_multiplier/(self.noise_multiplier + 0.0001 * steps))) / batch_size
        # self.noise_amplitude = self.noise_multiplier / batch_size
        actor_loss, prob, grad_norm = self.actor_update(states)
